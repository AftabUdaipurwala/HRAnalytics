...
suppressPackageStartupMessages(suppressMessages(suppressWarnings(library(randomForest))))
...
mod1 = randomForest(is_promoted ~ department + region + education + 
                      recruitment_channel + no_of_trainings + age + previous_year_rating + 
                      length_of_service + KPIs_met..80. + awards_won. + avg_training_score ,data = train, mtry=4,ntree=500,do.trace=T)

library(e1071)

mod1 = naiveBayes(is_promoted ~ department + region + education + 
                      recruitment_channel + no_of_trainings + age + previous_year_rating + 
                      length_of_service + KPIs_met..80. + awards_won. + avg_training_score ,data = train, mtry=4,ntree=500,do.trace=T)
...
mod1
prediction=predict(mod1,newdata=train)
prediction
